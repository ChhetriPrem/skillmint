...
const { Keypair, PublicKey, SystemProgram, Transaction, TransactionInstruction, SYSVAR_RENT_PUBKEY, Connection } = require('@solana/web3.js');

// You need to provide these utility functions:
const { serializeCreateBadgeTemplateArgs } = require('../services/serialization');
...
if (e.response) {
  ...
  console.error("Pinata file upload error (response):", e.response.status, e.response.data);
} else if (e.request) {
...
throw new Error("Failed to upload file to Pinata: " + (e.message || "Unknown error"));
...
async function uploadJSONToPinata(json) {
  try {
    ...
    if (e.response) {
      console.error("Pinata JSON upload error:", e.response.status, e.response.data);
    } else {
...
exports.uploadTemplateMetadata = async (req, res) => {
  try {
    ...
    const { url: imageUrl, mimetype: imageType } = await uploadFileToPinata(imagePath, mimetype);
...
exports.storeTemplate = async (req, res) => {
  try {
    const { templateName, description, metadataUri, imageUrl, txSignature } = req.body;
    console.log("storeTemplate called with:", { templateName, description, metadataUri, imageUrl, txSignature });

    if (!templateName || !description || !metadataUri || !imageUrl || !txSignature) {
